...
function cleanAvatar(value) {
  ...
  // Accept Cloudinary/CDN/http URLs or small base64 data URLs generated by the current frontend.
  if (/^https?:\/\//i.test(raw)) return raw.slice(0, 500000);
...
async function handleSignup(req, res, next) {
  try {
    ...
    const avatarUrl = cleanAvatar(req.body?.avatarUrl || req.body?.profilePictureUrl);
...
router.put("/me", auth, async (req, res, next) => {
  try {
    ...
    if (typeof req.body?.avatarUrl !== "undefined" || typeof req.body?.profilePictureUrl !== "undefined") {
      const avatarUrl = cleanAvatar(req.body?.avatarUrl || req.body?.profilePictureUrl);
